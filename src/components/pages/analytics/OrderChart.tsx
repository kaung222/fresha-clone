...
import { useGetOrderStatistics } from '@/api/statistics/get-order-statistics';
...
const OrderChart = ({ className }: Props) => {
    const { getQuery } = useSetUrlParams();
    const type = getQuery('type');
    const { data: orderData, isLoading } = useGetOrderStatistics();
    if (isLoading) {
        return (
    ...
    //@ts-ignore
    const chartData = (type == "daily") ? dailyDataFormat(orderData, dayLabel) : monthDataFormat(orderData, monthlyLabel);
